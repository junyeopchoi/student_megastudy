...
FROM
    ...
    ON O.ORDER_GOODS_QTY >= 10;


# 문제 2) 2020년 동안 판매된 매출총액 조회하기.(가격 * 개수 + 배송비)
SELECT
	P.PRODUCT_NAME,
    P.PRODUCT_PRICE * O.ORDER_GOODS_QTY + P.PRODUCT_DELIVERY_PRICE AS ALLPRICE
FROM
	T_ORDER O
    INNER JOIN T_PRODUCT P
    ON O.ORDER_DATE between '2020-01-01' AND '2020-12-31'
...
SELECT
	P.PRODUCT_CODE,
    P.PRODUCT_NAME,
    SUM(O.ORDER_GOODS_QTY) ALLPRICE
FROM
	T_PRODUCT P
...
SELECT
	M.REGIDENCE,
	SUM(O.ORDER_GOODS_QTY) TOTALPRICE
FROM 
	T_MEMBER M
	INNER JOIN T_ORDER O
    ON O.MEMBER_ID = M.MEMBER_ID
GROUP BY M.REGIDENCE
...
FROM
	T_MEMBER M
    INNER JOIN T_ORDER O
    ON M.MEMBER_ID = O.MEMBER_ID AND O.DELIVERY_STATE = '배송완료';
...
FROM
	T_ORDER O
    INNER JOIN T_PRODUCT P
    ON O.PRODUCT_CODE = P.PRODUCT_CODE AND O.DELIVERY_STATE = '배송준비중'
...
SELECT
	P.PRODUCT_CODE,
    P.PRODUCT_NAME,
    O.ORDER_GOODS_QTY * P.PRODUCT_PRICE TOTALPRICE
FROM
	T_PRODUCT P
    INNER JOIN T_ORDER O
...
FROM
	T_MEMBER M
    INNER JOIN T_ORDER O
    ON M.MEMBER_ID = O.MEMBER_ID AND M.MEMBER_NAME = '메르켈'
    INNER JOIN T_PRODUCT P
...
SELECT
	SUM(P.PRODUCT_PRICE * O.ORDER_GOODS_QTY) TOTAL
FROM
	T_MEMBER M
    INNER JOIN T_ORDER O
    ON M.MEMBER_ID = O.MEMBER_ID AND M.MEMBER_NAME = '메르켈'
...
SELECT
	M.MEMBER_ID,
    M.MEMBER_NAME,
    P.PRODUCT_PRICE * O.ORDER_GOODS_QTY TOTAL
FROM
	T_MEMBER M
    INNER JOIN T_ORDER O
...
# 문제 11) '사용자'별로 주문금액 총합이 1000만원 이상인 회원의 이름을 오름차순으로 정렬하여 조회하기.
SELECT
	M.MEMBER_NAME
FROM
	T_MEMBER M
    ...
    INNER JOIN T_PRODUCT P
    ON P.PRODUCT_CODE = O.PRODUCT_CODE
    WHERE P.PRODUCT_PRICE * O.ORDER_GOODS_QTY >= 10000000
GROUP BY M.MEMBER_ID
...
FROM
    ...
    ON P.PRODUCT_CODE = O.PRODUCT_CODE
GROUP BY M.MEMBER_ID
ORDER BY P.PRODUCT_PRICE * O.ORDER_GOODS_QTY DESC
LIMIT 3;
